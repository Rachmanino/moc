...
from .kernel import LlamaMoC_triton
...
class LlamaMoC_posttopk_autograd(nn.Module):
    def __init__(
        self,
        hidden_size: int,
        intermediate_size: int,
    ...
    def forward(self, x):
        ...
        act_c, indices = torch.topk(act, self.act_channels, dim=-1, largest=True, sorted=False)
        v_c = self.up_proj(x).gather(-1, indices)
        scaled_v_c = act_c * v_c
        scaled_v = torch.zeros_like(gate).scatter_(-1, indices, scaled_v_c)
        return self.down_proj(scaled_v)
...
class LlamaMoC(torch.autograd.Function):
    # For simplicity, assume no dropout(common for PT).
    @staticmethod 
    def forward(ctx, x, gate_w, fc_w, proj_w, act_channels: int):
        ...
        act_c = torch.ops.aten.gelu(gate_c) 
        v_c = torch.matmul(x, fc_w.t()).gather(-1, indices)
        scaled_v_c = act_c * v_c
        scaled_v = torch.zeros(ctx.shape, device=scaled_v_c.device, dtype=scaled_v_c.dtype).scatter_(-1, indices, scaled_v_c)
        ctx.save_for_backward(x, gate_w, fc_w, proj_w, gate_c, act_c, v_c, scaled_v_c, indices.to(torch.uint16))
        return torch.matmul(scaled_v, proj_w.t())
    
    @staticmethod
    def backward(ctx, grad_output):
        x, gate_w, fc_w, proj_w, gate_c, act_c, v_c, scaled_v_c, indices = ctx.saved_tensors
        ...
        grad_proj_w =  torch.bmm(grad_output.transpose(1, 2), scaled_v).sum(0)
        # grad_proj_w = torch.ops.aten.einsum('bsd, bse -> de', [grad_output, scaled_v])
        grad_v_c = act_c * grad_scaled_v_c
        grad_act_c = v_c * grad_scaled_v_c
        grad_gate_c = torch.ops.aten.gelu_backward(grad_act_c, gate_c) # gelu op. kernel
        grad_gate = zero.scatter(-1, indices, grad_gate_c)
        grad_v = zero.scatter(-1, indices, grad_v_c)
        grad_gate_w = torch.bmm(grad_gate.transpose(1, 2), x).sum(0)
        # grad_gate_w = torch.ops.aten.einsum('bse, bsd -> ed', [grad_gate, x])
        grad_fc_w = torch.bmm(grad_v.transpose(1, 2), x).sum(0)
        # grad_fc_w = torch.ops.aten.einsum('bse, bsd -> ed', [grad_v, x])
        grad_x = torch.matmul(grad_gate, gate_w) + torch.matmul(grad_v, fc_w)
        return grad_x, grad_gate_w, grad_fc_w, grad_proj_w, None


class LlamaMoC_real(nn.Module): 
    def __init__(
        self,
        hidden_size: int,
        intermediate_size: int,
    ...
    def forward(self, x):
        'Only keep `act_channels` channels with largest gate values'
        # No dropout
        return LlamaMoC.apply(x, self.gate_proj.weight, self.up_proj.weight, self.down_proj.weight, self.act_channels)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.mlp = LlamaMoC_triton(
            hidden_size=self.hidden_size,
            intermediate_size=config.intermediate_size,
            hidden_act=config.hidden_act,
            act_channels=config.act_channels
        )
    ...
    def forward(
        ...
        #     self.input_layernorm, hidden_states, use_reentrant=True
        # )
        ...
        residual = hidden_states
        # hidden_states = torch.utils.checkpoint.checkpoint(
        #     self.post_attention_layernorm, hidden_states, use_reentrant=True
        # )
...
class LlamaOriginalDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaAttention(config=config)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # self.layers = nn.ModuleList([LlamaOriginalDecoderLayer(config)] + [LlamaDecoderLayer(config) for _ in range(config.num_hidden_layers-1)])
        # self.layers = nn.ModuleList([LlamaOriginalDecoderLayer(config)] + [LlamaDecoderLayer(config) for _ in range(config.num_hidden_layers-1)])
...
class LlamaForCausalLM_MoC(LlamaPreTrainedModel):
